...
import { removeNewPost, setUserPosts } from "../../utils/reducers/postReducer";
...
function Home() {
  ...
  const newPost = useSelector((state)=> state?.userPosts?.newPost);
  ...
  return (
    <>
      <div className="md:ml-auto">
        ...
        >
          {posts?.map((post) => {
            return <SinglePost likeModal={likesModal} setLikePost={setLikePost} setSelectedPost={setSelectedPost}  key={post._id} postData={post} openEditor={openEditor} />;
          })}
          {loading ? (
      ...
      <button ref={openEditor} data-modal-target="default-modal" data-modal-toggle="default-modal" className="hidden" type="button" />
      ...
      <button ref={closeEditor} type="button" className="hidden" data-modal-hide="default-modal" />




<button ref={likesModal} type="button" data-modal-target="crypto-modal" data-modal-toggle="crypto-modal" className="hidden" />
<div id="crypto-modal" tabIndex="-1" aria-hidden="true" className="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div className="relative w-full max-w-md max-h-full">
        <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
            <button type="button" ref={closeLikeModal} className="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="crypto-modal">
